...
function renderResponseCard(payload, mode) {
  if (mode === "register" || mode === "admin-register") {
    const isPending = mode === "admin-register"
...
function initCaptureWorkflow(prefix, endpoint, bodyBuilder) {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if ((prefix === "login" || prefix === "admin-login") && payload.token) {
        localStorage.setItem("si_token", payload.token)
        window.location.href = prefix === "admin-login" ? "/admin" : "/user/dashboard"
        return
      }
...
function modeLabel(prefix, message) {
  return prefix === "register" ? `Enrollment in progress. ${message}` : `Authentication in progress. ${message}`
}
