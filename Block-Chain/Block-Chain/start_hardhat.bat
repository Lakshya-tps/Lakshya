...
if exist "%POWERSHELL%" (
  rem Run through PowerShell so we can stream output to console + write the log.
  rem `*>&1` merges all PowerShell streams into stdout, so native stderr won't surface as NativeCommandError records.
  "%POWERSHELL%" -NoProfile -Command "& { npx hardhat node *>&1 | Tee-Object -FilePath '%HARDHAT_LOG%'; exit $LASTEXITCODE }"
) else (
