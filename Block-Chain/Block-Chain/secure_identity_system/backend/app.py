...
def register_admin():
    ...
    created_admin = get_user_by_email(payload.email, role="admin")
    log_event(
        payload.email,
        "admin_register",
        True,
        f"quality={analysis.quality_score}; chain={blockchain_status}{'; tx=' + tx_hash if tx_hash else ''}",
        role="admin",
    )
    ...
    return api_success(
        ...
        authenticated=True,
        token=jwt_token,
        user={"id": user_id, **serialize_user(created_admin)},
        quality=serialize_quality(analysis),
        blockchain={
            **current_blockchain_state(),
            "write_status": blockchain_status,
